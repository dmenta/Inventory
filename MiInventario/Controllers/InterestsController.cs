...
namespace MiInventario.Controllers {
  ...
  public class InterestsController : BaseController {
    ...
    public ActionResult DateCharts(DateGrouping? grouping, string itemID, bool? accumulative) {
      var reproduccionesDB = Database.ReproduccionesItems
          .Where(p => p.Reproducciones.IdUsuario == Username)
          .Select(s => s.ItemID)
          .Distinct()
          .ToList();
    ...
    public ActionResult InterestsChart(DateGrouping grouping, string itemID, bool accumulative) {
      ...
      var reproduccionesDB = Database.ReproduccionesItems.Where(p => p.Reproducciones.IdUsuario == Username)
...
ForeColor=""White""
    ...
    public ActionResult DateCapsule(DateGrouping grouping, string idCapsula) {
      ...
      var reproduccionesDB = Database.ReproduccionesItems.Where(p => p.Reproducciones.IdUsuario == Username && (string.IsNullOrEmpty(idCapsula) || p.Reproducciones.IdCapsula == idCapsula))
      ...
      var filas = reproduccionesDB
                  ...
                  .Select(s => new FechaTotalViewModel {
                    ...
                    Items = s.GroupBy(u => u.ItemID).Select(v => new Models.ItemInventoryViewModel { Quantity = v.Sum(w => w.Cantidad), CurrentItem = ItemsXml.Single(p => p.ItemID == v.First().ItemID) })
                  }).ToList();
    ...
    public ActionResult _DateTotal(DateGrouping grouping, int periods) {
      ...
      var reproduccionesDB = Database.ReproduccionesItems.Where(p => p.Reproducciones.IdUsuario == Username && DbFunctions.TruncateTime(p.Reproducciones.Fecha).Value >= startDate)
          .GroupBy(r => new { Fecha = DbFunctions.TruncateTime(r.Reproducciones.Fecha).Value, IdCapsula = r.Reproducciones.IdCapsula, ItemID = r.ItemID })
          .Select(s => new {
            IdCapsula = s.Key.IdCapsula,
            Fecha = s.Key.Fecha,
      ...
      //var differentItems = reproduccionesDatabase.Select(h => h.ItemID).Distinct().Select(i => new Models.ItemViewModel { CurrentItem = ItemsXml.Single(j => j.ItemID == i) }).OrderBy(k => k.CurrentItem.Order);

      var totals = reproduccionesDB
       .GroupBy(q => q.ItemID)
       .Select(i => new Models.ItemInventoryViewModel { CurrentItem = ItemsXml.Single(j => j.ItemID == i.Key), Quantity = i.Sum(j => j.Cantidad) }).OrderBy(k => k.CurrentItem.Order);
    ...
    public FileResult PercentageRarityChart(DateGrouping grouping, bool percentage) {
      ...
      var dbData = Database.ReproduccionesItems.Where(p => p.Reproducciones.IdUsuario == Username)
    ...
    public ActionResult LastSpawn() {
      DateTime fechaMax = Database.Reproducciones.Where(p => p.IdUsuario == Username).DefaultIfEmpty().Max(q => q == null ? DateTime.MinValue : q.Fecha);
      ...
      var reproducciones = Database.ReproduccionesItems
          .Where(p => p.Reproducciones.Fecha == fechaMax && p.Reproducciones.IdUsuario == Username)
    ...
    public ActionResult TotalCapsule(string id) {
      var reproducciones = Database.ReproduccionesItems.Where(p => (id == null || p.Reproducciones.IdCapsula == id) && p.Reproducciones.IdUsuario == Username)
                  .OrderBy(q => q.ItemID)
                   .Select(s => new {
    ...
    public ActionResult TotalByItem() {
      var totalesDB = Database.Reproducciones.Where(p => p.IdUsuario == Username)
