...
ForeColor=""White""
    ...
    public ActionResult DateCapsule(DateGrouping grouping, string capsuleCode) {
      ...
      var filas = reproduccionesDB
                  ...
                  .Select(s => new FechaTotalViewModel {
                    ...
                    Items = s.GroupBy(u => u.ItemId).Select(v => new Models.ItemInventoryViewModel { Quantity = v.Sum(w => w.Quantity), CurrentItem = ItemsXml.Single(p => p.ItemId == v.First().ItemId) })
                  }).ToList();
    ...
    public ActionResult _DateTotal(DateGrouping grouping, int periods) {
      ...
      var totals = reproduccionesDB
       .GroupBy(q => q.ItemId)
       .Select(i => new Models.ItemInventoryViewModel { CurrentItem = ItemsXml.Single(j => j.ItemId == i.Key), Quantity = i.Sum(j => j.Quantity) }).OrderBy(k => k.CurrentItem.Order);
    ...
    public ActionResult LastSpawn() {
      ...
      model.Capsules = reproducciones
          ...
          .Select(s => new Models.Capsules.ContentsViewModel {
            ...
            Items = s.OrderBy(x => x.ItemId).Select(t => new Models.ItemInventoryViewModel {
