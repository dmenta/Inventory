...
namespace MiInventario.Controllers {
  [Authorize]
  public class Capsules2Controller : BaseController {
    ...
    [HttpGet]
    public JsonResult GetCapsules() {
      using (InventarioEntities db = new InventarioEntities()) {
        string user = User.Identity.GetUserName();

        DateTime start = DateTime.Now;
        var capsulesDB = db.Capsulas.Where(p => p.IdUsuario == user).Select(q=>new {CapsuleDB=q, ItemsDB=q.CapsulasItems}).ToList();

        var capsules = capsulesDB.Select(p => new CapsuleModel {
          IdCapsula = p.CapsuleDB.IdCapsula,
          CapsuleItem = ItemsXml.Single(q => q.ItemID == p.CapsuleDB.ItemID),
          Descripction = p.CapsuleDB.Descripcion,
          Items = ItemsXml.Join(p.ItemsDB, it => it.ItemID, ci => ci.ItemID, (it, ci) => new CapsuleItemModel { CurrentItem = it, Quantity = ci.Cantidad })
        }).ToList();
